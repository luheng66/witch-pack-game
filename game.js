...
const configSources = {
  itemsTable: ["./items.js", "./assets/data/items.js"],
  levelsTable: ["./levels.js", "./assets/data/levels.js"],
  levelItemsTable: ["./level-items.js", "./assets/data/level-items.js"]
};

async function loadConfigTable(exportName) {
  const version = Date.now();

  for (const filename of configSources[exportName]) {
    try {
      const mod = await import(`${filename}?v=${version}`);
      const table = mod[exportName];
      if (Array.isArray(table)) {
        console.log(`Loaded ${exportName} from ${filename}`);
        return table;
      }
...
async function loadConfigs() {
  const items = await loadConfigTable('itemsTable');
  const levels = await loadConfigTable('levelsTable');
  const levelItems = await loadConfigTable('levelItemsTable');

  itemsTable = items && items.length > 0 ? items : defaultItemsTable;
  levelsTable = levels && levels.length > 0 ? levels : defaultLevelsTable;
  levelItemsTable = levelItems && levelItems.length > 0 ? levelItems : defaultLevelItemsTable;

  console.log(`Config ready: ${itemsTable.length} items, ${levelsTable.length} levels, ${levelItemsTable.length} level-items`);
}
...
const initGrid = () => {
  ...
  els.grid.style.gridTemplateColumns = `repeat(${COLS}, ${CELL}px)`;
  els.grid.style.gridTemplateRows = `repeat(${ROWS}, ${CELL}px)`;
  
  const gridWidth = COLS * CELL + (COLS - 1) * GAP;
  const gridHeight = ROWS * CELL + (ROWS - 1) * GAP;
  
  els.gridWrap.style.width = `${gridWidth + 36}px`;
  els.gridWrap.style.height = `${gridHeight + 36}px`;
  els.placedLayer.style.width = `${gridWidth}px`;
  els.placedLayer.style.height = `${gridHeight}px`;
  els.departBtn.style.width = `${gridWidth + 36}px`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (saved) {
      ...
      levelIndex = Number.isInteger(p.levelIndex) ? p.levelIndex : p.currentLevel || 0;
    }
  } catch (e) { levelIndex = 0; }
  if (levelIndex < 0 || levelIndex >= getTotalLevels()) levelIndex = 0;
  const level = getCurrentLevel(levelIndex);
